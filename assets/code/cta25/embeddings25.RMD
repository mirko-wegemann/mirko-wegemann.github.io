...
if (!require('textstem')) install.packages("textstem")            # data wrangling
...
```{r}
not_in_emb <- vocab %>%
  filter(!term %in% embedding_vct$V1)

print(paste0(nrow(not_in_emb), " words of our MARPOR corpus are not represented in the pre-trained embeddings."))
not_in_emb <- not_in_emb[order(not_in_emb$term_count, decreasing=T),]

print(paste0("The most frequently used words which are not captured by the pre-trained embeddings are ", paste0(head(not_in_emb$term), collapse=", ")))
```
...
# add those words which do not exist in input embeddings with a 0
embeddings_na <- matrix(data = 0, nrow = nrow(not_in_emb), ncol = 300)

# set terms as rownames
rownames(embeddings_na) <- not_in_emb$term
...
head(sort(capital_cos_sim[,1], decreasing = TRUE), 10)
```
Yes, it's Berlin!
...
```{r}
which_leaning = embeddings["democrat", , drop = FALSE] -
  embeddings["liberal", , drop = FALSE] +
  embeddings["conservative", , drop = FALSE]
leaning_cos_sim = sim2(x = embeddings, y = which_leaning, method = "cosine", norm = "l2")
head(sort(leaning_cos_sim[,1], decreasing = TRUE), 10)
```
...
# which are close words to the term "migrant"?
...
# 6. now, we have several embeddings for our target word; to get a single estimate, aggregate column
context_wv <- colMeans(context_dem)
```


We can scrutinize how different actors are talking about a keyword of interest.
```{r}
...
```
We can learn quite something from this. Growth and innovation are quite close to each other, as social and environmental are. Both are on the opposite ends of the two-dimensional space, so there may be a trade-off between them. 
...
text <- c("The President greets the press in Chicago", "Obama speaks to the media in Illinois", "The band gave a concert in Japan.", "Obama performed on stage in China.")
dfm_example <- quanteda::tokens(text) %>% dfm()
...
print(df$sentence_context[119])
print(df$sentence_context[120])

mat_marpor <- as.matrix(dem_marpor)

mat_marpor2 <- mat_marpor[c(2,25,26,27,119,120),]
...
```{r}
# create training data (let's use same IDs as yesterday)
