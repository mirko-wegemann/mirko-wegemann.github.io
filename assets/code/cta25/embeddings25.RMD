...
df_visualization  <- data.frame(word = rownames(emb_df_sub2),  
                  ...
                  x = visualization[, 1], y = visualization[, 2],  
...
```{r}
# create training data (let's use same IDs as in other tasks [not relevant in this workshop])
index_train <- readRDS("./train_ids.RDS") %>% as.numeric
index_test <- readRDS("./test_ids.RDS")  %>% as.numeric
...
model <- keras_model_sequential() %>%
  layer_embedding(
    input_dim = dim(features_embeddings)[1],  
    input_length = max_len,
    output_dim = dim(features_embeddings)[2], 
    weights = list(features_embeddings),
    trainable = F
  ) %>%
  layer_flatten() %>%
  layer_dense(units = 1, activation = "sigmoid")  
...
print(paste0("The accuracy in our validation set is ", acc <- history$metrics$val_acc[best_epoch] %>% round(3)))
```
Okay, we see that there is quite a large error. The loss in our validation set is 0.23. Even the accuracy is not that great. 9% of our cases are wrongly predicted. 
...
The pre-trained embeddings treat each word separately and disregard sentence structure and inter-word relationships which might help to classify our data correctly. 
...
# Multi-class prediction 
```{r}
# create training data (let's use same IDs as in other tasks [not relevant for this workshop])
index_train <- readRDS("./train_ids.RDS") %>% as.numeric
index_test <- readRDS("./test_ids.RDS")  %>% as.numeric
