...
# GPT for daily tasks
```{r}
prompt <- paste0("Does this sound alright in British English? Do not explain, just provide me with the improved wording: ", "The UK Parliament decided to introduce a binding women's quota for large companies with 250 employees or more. This means that the management boards of these companies must have at least 40 per cent female executives in future.  ")
gpt_prompt <- gptstudio::openai_create_chat_completion(prompt, model="gpt-4o-mini") 
(text <- gpt_prompt$choices[[1]]$message$content)
...
gpt_prompt <- gptstudio::openai_create_chat_completion(prompt, model="gpt-4o-mini") # more recent: gpt-4.5 but also more expensive
df$sentence_context[4]
