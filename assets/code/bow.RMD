...
Let's do a topic model on a small subset of text first (5% of the text). This still takes 3 minutes.
```{r}
start <- Sys.time()
set.seed(3145)
...
(eval_mat <- table(docvars(test,"label"),test_predictions))
```
...
head(test_predictions,5)
(eval_mat2 <- table(docvars(test,"label"),test_predictions))
```
```{r}
print(paste0("There are ", eval_mat[[2]] + eval_mat[[3]], " wrong predictions"))
```

That's already an improvement, and that concludes our session. 